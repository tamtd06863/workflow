...
<body>
  ...
  <script>
    ...
    marker.on('dragend',function(){
      ...
      window.parent.postMessage(JSON.stringify({lat:p.lat,lng:p.lng}),'*');
    });
    ...
    window.addEventListener('message',function(e){
      try{var d=JSON.parse(e.data);window.moveMarker(d.lat,d.lng);}catch(err){}
    });
  </script>
</body>
</html>`;
...
export function LocationPickerModal({ visible, onClose, onConfirm, initialLat, initialLng, initialName }: Props) {
  ...
  // Listen for drag events from iframe
  useEffect(() => {
    async function handleMessage(e: MessageEvent) {
      try {
        const data = JSON.parse(e.data as string) as { lat: number; lng: number };
  ...
  function selectResult(result: NominatimResult) {
    ...
    setShowResults(false);
    setQuery('');
    iframeRef.current?.contentWindow?.postMessage(JSON.stringify({ lat, lng }), '*');
  }
        ...
        <View style={{ flex: 1 }}>
          ...
          {showResults && results.length > 0 && (
            <View
              ...
              style={{ position: 'absolute', top: 0, left: 16, right: 16, zIndex: 10 }}
            >
              {results.map((r, i) => (
                <Pressable
                  key={r.place_id}
